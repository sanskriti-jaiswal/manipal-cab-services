  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        setCaptain(data.captain) // ✅ Corrected state update
        localStorage.setItem('token', data.token)
        navigate('/captain-home')
      } else {
        setError('Signup failed. Please try again.') // Handle non-201 responses
      }
    } catch (error) {
    ...
    // ✅ Reset form fields only if signup is successful
    setEmail('')
    setFirstName('')
    setLastName('')
    setPassword('')
